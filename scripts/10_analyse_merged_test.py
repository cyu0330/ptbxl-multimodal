# scripts/08_analyse_merged_test.py
...
def main():
    ...
    m_base = compute_metrics(y_true, y_prob_base, threshold=args.threshold)
    for k, v in m_base.items():
        print(f"  {k}: {v}")

    # ---- Demo ----
    y_prob_demo = df[[f"y_prob_{lbl}_mm" for lbl in ecg_labels]].values.astype(np.float32)

    print("\n[ECG+Demo][TEST] metrics:")
    m_demo = compute_metrics(y_true, y_prob_demo, threshold=args.threshold)
    for k, v in m_demo.items():
        print(f"  {k}: {v}")
    ...
    if "y_true_AF" in df.columns and "y_prob_AF" in df.columns:
        ...
        m_af = compute_metrics(y_true_af, y_prob_af, threshold=args.threshold)
        for k, v in m_af.items():
            print(f"  {k}: {v}")
    else:
